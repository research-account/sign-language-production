    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) \
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                # print(f"length of one batch: {len(batch)}")
                # for one_sample in batch:
                #     print(type(one_sample))
                #     # print(one_sample)
                #     token_data = one_sample[0]
                #     skeleton_data = one_sample[1]
                #     directory_data = one_sample[2]
                #     print("____"*5)
                #     print(f"size of token data: {type(token_data)}")
                #     # print(f"token data: {token_data}")
                #     print(f"len(token_data): {len(token_data)}")
                #     print(f"len(token_data[0]: {len(token_data[0])}")
                #     print(f"len(token_data[0][0]): {len(token_data[0][0])}")
                #     print(f"token_data[0]: {token_data[0]}")
                #     print(f"len(token_data[1]: {len(token_data[1])}")
                #     print(f"token_data[1]): {token_data[1]}")
                #     print("____"*5)
                #     # print(f"skeleton data: {skeleton_data}")
                #     print(f"skeleton data dimensions=>")
                #     print(f"len(skeleton_data): {len(skeleton_data)}")
                #     for index, row in enumerate(skeleton_data):
                #         print(f"len(skeleton_data[{index}]): {len(row)}")
                #     print(f"len(skeleton_data[{index}][0]): {len(skeleton_data[index][0])}")
                #     print(f"len(skeleton_data[{3}][0]): {len(skeleton_data[3][0])}")
                #
                #     print(f"skeleton_data[{index}][0]: {skeleton_data[index][0]}")
                #     print("____"*5)
                #     print(f"directory_data: { directory_data }")
                #     break
...
def train(cfg_file: str, ckpt=None) -> None:
    ...
    outfile = open('train_example_sample.txt', 'w')
    for attribute in dir(example):
        print(f"{attribute}---------------", file=outfile)
        print(f"{getattr(example, attribute)}", file=outfile)
    outfile.close()

    # print(f"file path: {example.file_paths}, len(src): {len(example.src)}, len(trg):{len(example.trg)}")
    # print(f"src[0]: {example.src[0]}, src[1]: {example.src[1]}")
    # print(f"len(trg[0]): {len(example.trg[0])}")
    # print(f"trg[0][0]: {example.trg[0][0]}")
    ...
    test(cfg_file=cfg_file, ckpt=None)
    print("TEST COMPLETE---------------------------------------")
